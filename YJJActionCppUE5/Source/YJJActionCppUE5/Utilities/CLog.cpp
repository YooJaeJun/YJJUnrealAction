...
void CLog::Log(const TObjectPtr<UObject> InValue)
{
	FString str;
...
{
	UE_LOG(YJJ, Warning, TEXT("%s"), *InValue.StaticStruct()->GetName());
}
...
void CLog::Print(const TObjectPtr<UObject> InValue, const int32 InKey, const float InDuration, const FColor InColor)
{
	FString str;
...
void CLog::Print(const FString& InFileName, const FString& InFuncName, const int32 InLineNumber)
{
	...
	const FString str = FString::Printf(TEXT("%s, %s, %d"), *fileName, *InFuncName, InLineNumber);
