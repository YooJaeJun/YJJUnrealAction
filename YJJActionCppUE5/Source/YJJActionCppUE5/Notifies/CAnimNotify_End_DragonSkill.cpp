#include "Notifies/CAnimNotify_End_DragonSkill.h"
...
FString UCAnimNotify_End_DragonSkill::GetNotifyName_Implementation() const
{
	return TEXT("End_DragonSkill");
}

void UCAnimNotify_End_DragonSkill::Notify(
	...
	const FAnimNotifyEventReference& EventReference)
{
	...
	{
		CLog::Log(TEXT("[UCAnimNotify_End_DragonSkill] Mesh 소유 Actor 없음 — 노티 무시."));
		return;
	}
	...
	{
		CLog::Log(FString::Printf(TEXT("[UCAnimNotify_End_DragonSkill] 소유 폰이 ACDragon 아님 — %s."),
	...
	{
		CLog::Log(FString::Printf(TEXT("[UCAnimNotify_End_DragonSkill] DragonWeapon 미해결 — Dragon=%s."),
			*GetNameSafe(dragonScratch)));
		return;
	}

	const uint8 skillByteScratch = static_cast<uint8>(SkillType);
	weaponScratch->End_Skill(skillByteScratch);
}
